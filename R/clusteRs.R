...
#' @param range maximum number of clusters
#' @return overview of indices for number of clusters
#' @export
#' @importFrom factoextra get_dist get_clust_tendency hkmeans fviz_nbclust
#' @importFrom stats hclust dist na.omit var as.dist median complete.cases cov runif kmeans cutree
...
clusteRs <- function(dataset,range=10){
  ...
  #                                      function(x){stats::runif(length(x),
  #                                                        min(x),
  #                                                        max(x))}
  # ))
  #
  ...
  gapstatisticplot <- factoextra::fviz_nbclust(dff,hkmeans,  k.max = range, linecolor = "black", method="gap_stat", nboot=50) +
    ggplot2::labs(title=NULL, subtitle = "Gap statistic", xlab='K clusters') + ggplot2::theme_minimal()

  NbClustadapted <-function(data = NULL, diss=NULL, distance ="euclidean", min.nc=2, max.nc=15, method =NULL, index = "all", alphaBeale = 0.1)
  {
    ...
    else
    {
      ...
      jeu <- stats::na.omit(jeu1) # returns the object with incomplete cases removed
    ...
    Average.scattering <- function (cl, x)
    {
      ...
      for(j in 1:ncol(x))
        variance.matrix[j]=stats::var(x[,j])*(n-1)/n
    ...
    Index.Hubert<-function(x, cl)
    {
      ...
      for(j in 1:n)
        variance.matrix[j]=stats::var(P[,j])*(n-1)/n
      ...
      meanQ<-mean(Q)
      for(j in 1:n)
        variance.matrix[j]=stats::var(Q[,j])*(n-1)/n
    ...
    Index.sPlussMoins <- function (cl1,md)
    {
      ...
      for (u in 1:cn1) {
        ...
        du <- stats::as.dist(dmat[cl1 == u, cl1 == u])
        within.dist1 <- c(within.dist1, du)
        average.distance[u] <- mean(du)
        median.distance[u] <- stats::median(du)
        bv <- numeric(0)
        for (v in 1:cn1) {
          if (v != u) {
    ...
    Index.15and28  <- function (cl1,cl2,md)
    {
      ...
      for (w in 1:cn2) {
        ...
        dw <- stats::as.dist(dmat[cl2 == w, cl2 == w])
        within.dist2 <- c(within.dist2, dw)
        #average.distance[w] <- mean(dw)
        #median.distance[w] <- stats::median(dw)
        bx <- numeric(0)
        for (x in 1:cn2) {
          if (x != w) {
    ...
    Indice.ptbiserial <- function (x,md,cl1)
    {
      ...
      biserial.cor <- function (x, y, use = c("all.obs", "complete.obs"), level = 1)
      {
        ...
        use <- match.arg(use)
        if (use == "complete.obs") {
          cc.ind <- stats::complete.cases(x, y)
          x <- x[cc.ind]
          y <- y[cc.ind]
        }
        ind <- y == levs[level]
                            ...
                            method = "ward.D2", d = NULL, centrotypes = "centroids")
    {
      GAP <- function(X, cl, referenceDistribution, B, method, d, centrotypes)
      {
        ...
        simgap <- function(Xvec)
        {
          ...
          mi <- min(Xvec)
          set.seed(1)
          Xout <- stats::runif(length(Xvec), min = mi, max = ma)
          return(Xout)
        }
        ...
        for (bb in (1:B)) {
          ...
          if (bb == 1) {
            ...
            { set.seed(1)
              pp2 <- stats::kmeans(Xnew, ClassNr, 100)$cluster
            }
              ...
              pp2 <- stats::cutree(hclust(dist(Xnew), method = method),
                            ClassNr)
            else stop("Wrong clustering method")
            if (ClassNr > 1) {
              for (zz in (1:ClassNr)) {
                Xuse <- X[pp == zz, ]
                Wk0 <- Wk0 + sum(diag(stats::var(Xuse))) * (length(pp[pp ==
          ...
          if (bb > 1) {
            ...
            if (ClassNr > 1) {
              for (zz in (1:ClassNr)) {
                Xuse2 <- Xnew[pp2 == zz, ]
                WkB[1, bb] <- WkB[1, bb] + sum(diag(stats::var(Xuse2))) *
                  length(pp2[pp2 == zz])/(dim(X)[1] - ClassNr)
              }
            }
            if (ClassNr == 1) {
              WkB[1, bb] <- sum(diag(stats::var(Xnew)))
            }
          }
        }
        Sgap <- mean(log(WkB[1, ])) - log(Wk0)
        Sdgap <- sqrt(1 + 1/B) * sqrt(stats::var(log(WkB[1, ]))) * sqrt((B -
                                                                    1)/B)
    ...
    for (nc in min_nc:max_nc)
    {
      ...
      {
        cl1 <- stats::cutree(hc, k=nc)
        cl2 <- stats::cutree(hc, k=nc+1)
        clall <- cbind(cl1, cl2)
        clmax <- stats::cutree(hc, k=max_nc)


        if (nc >= 2)
        {
          cl0 <- stats::cutree(hc, k=nc-1)
      ...
      {
        set.seed(1)
        cl2 <- stats::kmeans(jeu,nc+1)$cluster
        set.seed(1)
        clmax <- stats::kmeans(jeu,max_nc)$cluster
        if (nc > 2)
        {
          set.seed(1)
          cl1 <- stats::kmeans(jeu,nc)$cluster
          clall <- cbind(cl1, cl2)
          set.seed(1)
          cl0 <- stats::kmeans(jeu,nc-1)$cluster
          clall1 <- cbind(cl0, cl1, cl2)
        }
        if (nc == 2)
        {
    ...
    if(any(indice == 31) || (indice == 32))
    {
      ...
      if (any(method == 1) || (method == 2) || (method == 3) || (method == 4) ||
          (method == 5) || (method == 6) || (method == 7)||(method == 9))
        partition<- stats::cutree(hc, k=j)

      else
      {
        set.seed(1)
        partition<-stats::kmeans(jeu,j)$cluster
      }
        ...
        ||(indice==28)||(indice==30))
    {
      if (any(method == 1) || (method == 2) || (method == 3) || (method == 4) ||
        ...
        partition<- stats::cutree(hc, k=best.nc)

      else
      {
        set.seed(1)
        partition<-stats::kmeans(jeu,best.nc)$cluster
      }
