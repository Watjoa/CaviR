...
inteRplot <- function(model,pred,mod,outcome = 'outcome',
  ...
  meanslope <- paste('Mean (',simslopes[which(simslopes$rn=='(m)'),'est'],
                     simslopes[which(simslopes$rn=='(m)'),'stars'],')',sep="")
  ...
  maxpoint <- which(heyy[,2]==max(heyy[,2]))
  heyy$parts <- 'low'
  heyy[meanpoint:maxpoint,'parts'] <- 'high'
  ...
  linesmod <- heyy[,2]
  heyy$significance <- 'no'
  heyy[which(heyy$`Pr(>|t|)`<0.05),'significance'] <- 'yes'

  minJNI <- suppressWarnings(min(heyy[which(heyy$significance=="no"),2],na.rm=TRUE))
  maxJNI <- suppressWarnings(max(heyy[which(heyy$significance=="no"),2],na.rm=TRUE))

  intervaljn <- c(minJNI,maxJNI)
  ...
  linesvalues <- paste(mod,'[',paste(linesmod,sep="", collapse=","),']',sep="")
  ...
  ggpredictions_ols3BIG = data.frame(ggeffects::ggpredict(model,
                                                          c(predvalues,
                                                            linesbigvalues)))
  ggpredictions_ols3BIG$group <- as.character(ggpredictions_ols3BIG$group)
  ggpredictions_ols3BIG$group <- as.numeric(ggpredictions_ols3BIG$group)
  ggpredictions_ols3BIG$groupname <- as.factor(ggpredictions_ols3BIG$group)
  levels(ggpredictions_ols3BIG$groupname) <- numbers$labelss
  ggpredictions_ols3BIG$predname <- as.factor(ggpredictions_ols3BIG$x)
...
if('Inf' %in% intervaljn == TRUE){
  ...
  #
  # # test whether it is inside or outside the interval
  # dfmodel <- model$model
  # model <-  model
  # pred <-  pred
  # modx <- modx
  ...
  # interactionsIn <- relevantInteractions[which(relevantInteractions %in% ivs)]
  #
  # meanmodxtest <- mean(c(x1,x2))
  # namestest <- c('test')
  # numberstest <- c(meanmodxtest)
  # names(numberstest) <- namestest
  # modxValstest <- numberstest #waarden voor -SD, M, +SD OK
  # bs <- stats::coef(model)
  # V <- stats::vcov(model)
  #
  # bmodx <- NULL
  ...
  # bmodx <- bs[interactionsIn]
  #
  # bsimpletest <- bpred + bmodx * modxValstest
  # covbsimpletest <- cbind(1, modxValstest^2, 2 * modxValstest) %*%
  #   c(V[pred, pred], V[names(bmodx), names(bmodx)], V[pred, names(bmodx)])
  #
  # tbsimpletest <- bsimpletest/sqrt(covbsimpletest)
  # dfresidualtest <-mean(unique(summary(model)$coefficient[,3]))
  # dfresidualtest <- as.integer(dfresidualtest)
  # testSlopestest <- data.frame( modx = modxValstest, b = bsimpletest,
  #                               se = sqrt(covbsimpletest), t = tbsimpletest,
  #                               p = 2 * stats::pt(abs(tbsimpletest),
  #                                                 df = dfresidualtest, lower.tail = FALSE))
  #
  # colnames(testSlopestest) <- c(deparse(modx), "slope", "Std. Error", "t value", "Pr(>|t|)")
  # heytest <- round(testSlopestest,4)
  # heyytest <- data.table::as.data.table(heytest,keep.rownames=TRUE)
  # heyytest
  #
  # if (intervalcheck == "FALSETRUE" & heyytest$`Pr(>|t|)`>0.05) {
  #
  #   intervalcheck <- "FALSETRUE"
  #   numbers[which(numbers$labelss=="lower significance"),'numbers'] <- NA
  #   numbers <- numbers[!is.na(numbers$numbers),]
  #
  # } else if (intervalcheck == "FALSETRUE" & heyytest$`Pr(>|t|)`< 0.05){
  #   intervalcheck <- "TRUEFALSE"
  #   numbers[which(numbers$labelss=="lower significance"),'numbers'] <- NA
  #   numbers[which(numbers$labelss=="higher significance"),'labels'] <- paste('lower boundary: ]',format(round(higher,2), nsmall = 2),sep="")
  #   numbers[which(numbers$labelss=="higher significance"),'labelss'] <- 'lower significance'
  #   numbers <- numbers[!is.na(numbers$numbers),]
  #
  # } else if (intervalcheck == "TRUEFALSE" & heyytest$`Pr(>|t|)`>0.05){ # OK
  #   intervalcheck <- "TRUEFALSE"
  #   numbers[which(numbers$labelss=="higher significance"),'numbers'] <- NA
  #   numbers <- numbers[!is.na(numbers$numbers),]
  #
  # }else if (intervalcheck == "TRUEFALSE" & heyytest$`Pr(>|t|)`<0.05){
  #   intervalcheck <- "FALSETRUE"
  #   numbers[which(numbers$labelss=="higher significance"),'numbers'] <- NA
  #   numbers[which(numbers$labelss=="lower significance"),'labels'] <- paste('higher boundary: ',format(round(lower,2), nsmall = 2),'[',sep="")
  #   numbers[which(numbers$labelss=="higher significance"),'numbers'] <- NA
  #
  #   numbers <- numbers[!is.na(numbers$numbers),]
